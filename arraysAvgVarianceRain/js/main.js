function mean(town, str) {
    if (!town || !str) return -1
    let obj1 = {}
    str = str.split('\n')
    str.forEach(el => {
      obj1[el.slice(0, el.indexOf(':'))] = el.slice(el.indexOf(':') + 1)
    })
    for (let key in obj1){
      obj1[key] = obj1[key].split(',')
      for (let i = 0 ; i < obj1[key].length ; i++){
        obj1[key][i] = parseFloat(obj1[key][i].slice(4))
      }
    }
    if (!obj1[town]) return -1
    return obj1[town].reduce((a,el) => a + el, 0) / 12
  }
  ...
  function variance(town, str) {
    ...
    let avg = mean(town, meanStr)
    if (!obj1[town]) return -1
    return obj1[town].reduce((a,el) => a + Math.abs(el - avg)**2 ,0) /12
  }
