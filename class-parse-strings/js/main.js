function travel(r, zipcode) {
    ...
    class Storage{
      ...
      address(){
        ...
        this.nums = num.trim()
        this.zip = this.str.slice(-8)
        this.street = this.str.slice(this.nums.length + 1, -9)
        
      }
    }
    
    r = r.split(',')
    r = r.map(str => new Storage(str))
    r.forEach(address => address.address())
    let streets = ''
    let nums = ''
    
    r.forEach(address => {
      if (address.zip === zipcode){
        if (!streets) streets = address.street
        else {streets += ',' + address.street}
        if (!nums) nums = address.nums
        else {nums += ',' + address.nums}
...
let code = "OH 43071,NY 56432,ZP 32908,AE 56210,RE 13000,EX 34342,SW 43098,AA 45521,GG 30654,ZP 32908,AE 56215,RE 13200,EX 34345,"
...
code.forEach(el => console.log(travel(ad, el)))